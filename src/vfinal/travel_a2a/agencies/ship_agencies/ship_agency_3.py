...
def search_trips(origin: str, destination: str, date: str | None, quality: str | None) -> List[Dict[str, Any]]:
    ...
    csv_path = data_dir / "ship_agency_3.csv"
...
agent = Agent(
    ...
    - La calidad del boleto y la fecha no son requeridos en la consulta del usuario, si no se especifican puedes elegir el viaje mas barato para la informacion que el usuario si aporto, como destino Y origen
    """,
    tools=[search_trips],
)

a2a_app = to_a2a(agent, port=8026)